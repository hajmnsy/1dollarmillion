...
export function SolvencyCard({ accounting }: SolvencyCardProps) {
  ...
  return (
    <Card className="relative overflow-hidden border-white/10 bg-white/[0.02] p-6 shadow-xl ring-1 ring-emerald-500/20">
      <div className="relative">
        <div className="mb-4 flex items-center justify-between gap-3">
          <h3 className="text-sm font-bold text-white">{t("title")}</h3>
          <div className="inline-flex items-center gap-1.5 rounded-full bg-emerald-500/15 px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-emerald-300">
...
function Row({ label, value, accent = false }: { label: string; value: string; accent?: boolean }) {
  return (
    <div className="flex items-center justify-between text-xs">
      ...
      <span className={`font-mono font-semibold ${accent ? "text-emerald-400" : "text-white/80"}`}>
        {value}
      </span>
    </div>
  );
