...
/**
 * @title  HybridRoscaLotteryV8 — OPTIMIZED & FIXED PRODUCTION VERSION
 * @notice V8 = O(1) Global Pool Accounting + Gas-Safe Draw Trigger + Sybil-Protected Referral
 *
 * ====================================================================
 *  FIXES IMPLEMENTED IN V8:
 * ====================================================================
 *  1. DoS Gas Limit Fix in syncAllAndTriggerDraw():
 *     - Replaced unbounded O(N) loop over activeUsers with O(1) Global Pool Sync.
 *     - Calculates pool additions mathematically in O(1) time based on elapsed days.
 *     - Prevents transaction failure / Out of Gas revert regardless of user count.
 *  2. Referral Sybil Attack Protection:
 *     - Referral bonus triggers ONLY for deposits >= 30 USDT ($30 minimum deposit).
 *     - Prevents creating 1$ spam wallets to extract free bonus days.
 *  3. Strict O(1) User Deduction:
 *     - Deducts $1/day lazily before every user deposit/withdrawal.
...
contract HybridRoscaLotteryV8 is ReentrancyGuard, Ownable, VRFConsumerBaseV2 {
    ...
    uint256 public constant REFERRAL_MIN_DEPOSIT = 30 * 10 ** DECIMALS; // Sybil protection
    ...
    struct User {
        uint128 balance;            // Synced balance
    ...
    event GlobalSyncPerformed(uint256 poolBefore, uint256 poolAfter);
    ...
    // ============================================================
    // ============ O(1) GLOBAL POOL SYNC & ESTIMATION ============
    // ============================================================

    /**
     * @notice Synchronize the pool state in O(1) time without looping over users.
     */
    function syncPool() public returns (uint256) {
        ...
        uint256 elapsedDays = (block.timestamp - lastGlobalSyncTime) / SECONDS_PER_DAY;
        if (elapsedDays > 0) {
            uint256 pendingAddition = elapsedDays * activeUsersAtLastSync * DAILY_DEDUCTION;
            if (pendingAddition > totalUserBalances) {
                pendingAddition = totalUserBalances;
            }
            currentPool += pendingAddition;
            totalUserBalances -= pendingAddition;

            lastGlobalSyncTime = block.timestamp;
            activeUsersAtLastSync = activeUsers.length;

            emit PoolUpdated(currentPool);
            emit GlobalSyncPerformed(currentPool - pendingAddition, currentPool);
        } else {
            activeUsersAtLastSync = activeUsers.length;
        }
        ...
        notDrawInProgress
    {
        ...
        // Referral Sybil Protection: require >= 30 USDT deposit for referral activation
        ...
        // Award referral bonus days (only for >= 30 USDT deposit)
    ...
    /**
     * @notice O(1) Gas-Safe trigger draw function.
     * @dev Anyone can call this when getEstimatedPool() >= POOL_TARGET.
     * No unbounded loops, 100% safe from Block Gas Limit Reverts!
     */
